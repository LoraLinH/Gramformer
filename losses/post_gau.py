    ...
    def forward(self, points, st_sizes):
        ...
        if len(all_points) > 0:
            ...
            for dis, st_size in zip(dis_list, st_sizes):
                if len(dis) > 0:
                    # gau = -dis / (2.0 * self.sigma ** 2)
                    gau = -dis / (2.0 * 0.1 ** 2)
                    gau = torch.nn.functional.softmax(gau, dim=1)
                    gau = torch.sum(gau, dim=0).resize(self.wid, self.wid)
                    if self.use_bg:
                    ...
                    prob = None
                    gau = torch.zeros((self.wid, self.wid), dtype=torch.float32, device=self.device)
